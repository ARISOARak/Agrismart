...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    if (!validateEmail(email)) {
      Alert.alert('Erreur', 'Veuillez entrer un email valide (ex: user@example.com)');
      return;
    }

    if (!validatePhone(phone)) {
      Alert.alert('Erreur', 'Veuillez entrer un numéro de téléphone valide (8-20 chiffres)');
      return;
    }
    ...
    try {
      ...
      console.log('Erreur inscription:', error.response?.data);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.roleContainer}>
          <TouchableOpacity
            style={[styles.roleButton, role === 'agriculteur' && styles.roleButtonActive]}
            onPress={() => setRole('agriculteur')}
          >
            <MaterialCommunityIcons 
              name="sprout" 
              size={18} 
              color={role === 'agriculteur' ? GREEN_DARK : '#aaa'} 
            />
            <Text style={[styles.roleText, role === 'agriculteur' && styles.roleTextActive]}>
              Agriculteur
            </Text>
          </TouchableOpacity>
          ...
          >
            <MaterialCommunityIcons 
              name="brain" 
              size={18} 
              color={role === 'expert' ? GREEN_DARK : '#aaa'} 
            />
